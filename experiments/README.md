...
The Camera->Fog full-rebuild case is closed. Its remaining low-zoom cost is explained Fog geometry/raster workload, not an unresolved pathology. New work should open a separate case when the objective or trigger path differs; in particular, unit-driven Fog reveal hitches and manual Fog-toggle hitches are not covered by this closure.
