...
# STAR Phase 5 C1 — Vision geometry cache-hit attribution
#
# Measurement only. Runs the retained Optimization-A production tree with a
# process-local Vision probe. No production runtime source is modified.
...
RESULTS_ROOT="${RESULTS_ROOT:-results/phase5-vision-hit-attribution}"
...
{
    echo "experiment=Phase 5 C1 — Vision Geometry Cache-Hit Attribution"
    echo "run_id=$RUN_ID"
    ...
    echo "hub=offline"
    echo "mock_ai=off"
    echo "probe_scope=VisionSystem._visibility_for cache-hit internals only"
...
run_point() {
    ...
    CURRENT_SOCKET="/tmp/star-phase5-c1-${$}-${label}.sock"
    rm -f "$CURRENT_SOCKET"

    echo
    echo "============================================================"
    echo "Phase 5 C1 Vision hit attribution: $label"
    ...
    STAR_PHASE5_VISION_HIT_SAMPLE_EVERY="$ATTR_SAMPLE_EVERY" \
    uv run python tools/phase5_vision_hit_attribution.py \
        --skip-start \
        --mode real_time \
        --scenario "$SCENARIO" \
    ...
    if [[ ! -f "$point_dir/profile.json" ]]; then
        echo "WARNING: profile.json missing for $label; inspect logs"
        return
    fi
...
summary = {
    ...
    "sections": {
        name: sections[name]
        for name in ("VisionSystem", "AnimationSystem")
        if name in sections
    },
    "frame_metrics": {
        name: value
        for name, value in metrics.items()
        if name.startswith("phase5_c1_")
        or name.startswith("vision_geometry_cache_")
        or name in {
            "vision_dirty_units",
...
echo
echo "============================================================"
echo "Phase 5 C1 Vision hit attribution completed"
